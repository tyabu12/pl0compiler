...
static int lIndex[MAXLEVEL];    /* index[i]にはブロックレベルiの最後のインデックス */
...
void blockBegin(int firstAddr) { /* ブロックの始まり(最初の変数の番地)で呼ばれる */
    ...
    lIndex[level] = tIndex;     /* 今までのブロックの情報を格納         */
...
void blockEnd() {               /* ブロックの終りで呼ばれる             */
    level--;
    tIndex = lIndex[level];     /* 一つ外側のブロックの情報を回復       */
...
int fPars() {                   /* 現ブロックの関数のパラメタ数を返す   */
    return nameTable[lIndex[level-1]].u.f.pars;
}
